...
//    }
    ...
    public ResponseEntity<?> getAllCategoryItems(HttpServletRequest httpServletRequest,
        ...
        Map<String, Object> responseBody = new HashMap<>();
        log.info("getAllCategoryItems 메소드 호출");
        try {
            PagedModel<EntityModel<GetAllItemDTO>> getAllCategoryItemDTOPage = itemService.getAllCategoryItemsPage(categoryGender,categoryType, page, size, sort, order, optionSize);

            responseBody.put("items", getAllCategoryItemDTOPage);
            responseBody.put("status", HttpStatus.OK.value());
            responseBody.put("message", "성공적으로 조회했습니다.");
            return ResponseEntity.ok(responseBody);
